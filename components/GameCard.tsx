...
// Cast the custom element to any to avoid TypeScript errors with IntrinsicElements
...
const GameCard: React.FC<GameCardProps> = ({ game }) => {
  ...
  const gameTitleId = `game-title-${game.title.replace(/\s+/g, '-')}`;
  ...
  }, [previewType, game.realTimePreviewUrl]);
    ...
    switch (previewType) {
      case '3d':
        ...
        return (
          <>
            <div 
              className={`absolute inset-0 flex flex-col items-center justify-center text-gray-500 dark:text-gray-400 bg-gray-100 dark:bg-gray-900 transition-opacity duration-500 ease-in-out ${modelStatus !== 'loading' ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}
              role="status"
              aria-live="polite"
            >
            ...
            {modelStatus === 'error' && (
              game.fallbackImageUrl ? (
                ...
                <div 
                  ...
                  aria-live="polite"
                >
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-12 w-12 mb-2" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.5" aria-hidden="true">
            ...
            <ModelViewer
              ...
              camera-controls
              auto-rotate
              className={`w-full h-full group-hover:scale-110 transition-all duration-500 ease-in-out ${modelStatus === 'loaded' ? 'opacity-100' : 'opacity-0'}`}
      ...
      case 'video':
        return (
          <>
            <div 
              className={`absolute inset-0 flex items-center justify-center bg-gray-100 dark:bg-gray-900 transition-opacity duration-300 ${!isVideoLoading ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}
              role="status"
              aria-live="polite"
            >
              <svg className="animate-spin h-8 w-8 text-cyan-600" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" aria-hidden="true">
  ...
  return (
    <div
      className="group bg-white dark:bg-gray-800 rounded-xl overflow-hidden shadow-lg hover:shadow-2xl hover:shadow-cyan-500/50 transition-all duration-300 ease-in-out transform-gpu hover:-translate-y-1 hover:rotate-x-2 hover:-rotate-y-2 flex flex-col"
      role="article"
      ...
      <div className="p-6 flex flex-col flex-grow">
        ...
        >
          <div className="w-full h-48 mb-4 rounded-md border border-gray-200 dark:border-gray-700 overflow-hidden bg-gray-100 dark:bg-gray-900 flex items-center justify-center relative">
            {renderPreview()}
            <span className="absolute top-2 right-2 bg-cyan-100 text-cyan-800 text-xs font-semibold px-2.5 py-0.5 rounded-full dark:bg-cyan-900 dark:text-cyan-300 transition-opacity duration-300 group-hover:opacity-75">
              {t(game.category.toLowerCase() as keyof typeof translations.en)}
            </span>
          </div>
          <h3 id={gameTitleId} className="text-2xl font-bold text-gray-800 dark:text-white mb-2 group-hover:text-cyan-600 dark:group-hover:text-cyan-400 transition-colors duration-300">{game.title}</h3>
        </a>
        <p className="text-gray-600 dark:text-gray-400 text-base flex-grow mb-4">{game.description}</p>
